...
(function (WGo) {
    ...
    var TsumegoApi = WGo.extendClass(WGo.Player, function (config) {
        ...
        for (var key in TsumegoApi.default)
            if (this.config[key] === undefined && TsumegoApi.default[key] !== undefined) this.config[key] = TsumegoApi.default[key];
    ...
    Tsumego.prototype.showVariations = function (e) {
        ...
        for (var i = 0; i < e.node.children.length; i++) {
            if (e.node.children[i].move && e.node.children[i].move.c == this.turn && !e.node.children[i].move.pass)
                this.variationLetters.push({
                    type: "LB",
